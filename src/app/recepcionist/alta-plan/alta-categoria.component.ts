...
export class AltaPlanComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.GimnasioService.getServicesForId(this.data.id).subscribe(
      (respuesta) => {
        if (respuesta) {
  ...
  validarFormulario() {
    if (this.formPlan.invalid) {
      ...
      this.planService
        .agregarPlan(this.formPlan.value)
        .subscribe((respuesta) => {
          if (respuesta) {
            this.dialog
              ...
              .subscribe((cerrarDialogo: Boolean) => {
                if (cerrarDialogo) {
                  ...
                  this.router
                    .navigate([
                      "/registro",
                      respuesta.registroInsertado.idMem,
                      respuesta.registroInsertado.titulo,
                      respuesta.registroInsertado.precio,
                    ])
                    .catch((error) => {});
                } else {
  ...
  calculateTotalPay() {
    ...
    if (
      duracionControl &&
      (duracionControl.dirty || duracionControl.value !== null)
    ) {
