...
export class PagoInscripcionComponent implements OnInit {
  ...
  constructor(
    ...
    private payInscripcion: PagoInscripcionService,
  ...
  formatearFecha() {
    ...
    if (this.fecha.length === 2 && this.fecha.indexOf("/") === -1) {
      ...
      if (
        parseInt(anio, 10) < parseInt(anioActual, 10) ||
        (parseInt(anio, 10) === parseInt(anioActual, 10) &&
          parseInt(mes, 10) < parseInt(mesActual, 10))
      ) {
  ...
  private initConfig(): void {
    this.payPalConfig = {
      currency: "MXN",
      clientId:
        "AcXxpxeoadCNQcHM-pX-lS6w1A61T5tDNTXmOtaO2b-gtIyEibZmrgqG_EBNwv03t48H1-whDLWMJoPD",
